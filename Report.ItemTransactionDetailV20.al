report 50005 "Item Transaction Detail - V2.0"
{
    // #7314   09/10/2015      MOI           Desarrollo Activo/Inactivo
    //         14/02/2017      PLB           Implementada funcionalidad
    DefaultLayout = RDLC;
    RDLCLayout = './ItemTransactionDetailV20.rdlc';

    Caption = 'Item Transaction Detail for day';

    dataset
    {
        dataitem(Item; Item)
        {
            DataItemTableView = SORTING("No.") ORDER(Ascending) WHERE(Inactivo = CONST(false));
            ...
            column(ReportName__; REPORT_NAME)
            {
            }
            ...
            column(ParametroFecha; FromDate)
            {
            }
            ...
            column(MostrarDia; MD)
            {
            }
            column(MostrarPeriodo; MP)
            {
            }
            ...
            dataitem("Item Ledger Entry"; "Item Ledger Entry")
            {
                ...
                column(Company_Name___; CompanyInformation.Name)
                {
                }
                column(FechaReg; FechaReg)
                {
                }
                ...
                column(MontoActual; MontoActual)
                {
                }
                column(CantidadActual; CantidadActual)
                {
                }
                ...
                begin
                    ...
                    MontoActualOriginal := "Cost Amount (Actual)";
                    MontoActual := MontoActual + MontoActualOriginal;
                    CantidadActual += Quantity;

                    FechaReg := Format("Posting Date", 0, '<Month,2>/<Year4>');
    ...
    begin
        ...
        if MostrarPor = MostrarPor::"Por Día" then
            MD := 'true';
        if MostrarPor = MostrarPor::"Por Periodo" then
            MP := 'true';
    end;
    ...
    var
        ...
        BoolPeriodo: Boolean;
        MontoActual: Decimal;
        MontoActualOriginal: Decimal;
        CantidadActual: Decimal;
        CantidadActualOriginal: Decimal;
        vQuant: Decimal;
        vAmount: Decimal;
        MD: Text;
        MP: Text;
        MostrarPor: Option "Por Día","Por Periodo";
        REPORT_NAME: Label 'Inventory: details of movs.';
}
