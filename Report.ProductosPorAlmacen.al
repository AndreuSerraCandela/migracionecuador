report 55036 "Productos Por Almacen"
{
    // MOI - 11/12/2014 (#7694): Se tiene que volver a crear el layout debido a temas de imcompatibilidad de NAV2009R2 a NAV2013R2.
    DefaultLayout = RDLC;
    RDLCLayout = './ProductosPorAlmacen.rdlc';
    ...
    dataset
    {
        dataitem(Location; Location)
        {
            ...
            dataitem(Item; Item)
            {
                ...
                column(Item2_InventarioFecha_; InventarioFecha)
                {
                }
                column(EAN; EAN)
                {
                ...
                begin
                    ...
                    //EAN
                    itemcross_g.Reset;
                    itemcross_g.SetRange("Item No.", "No.");
                    itemcross_g.SetRange("Reference Type", itemcross_g."Reference Type"::"Bar Code");
                    if itemcross_g.FindFirst then
                        EAN := itemcross_g."Reference No.";
                end;
    ...
    var
        ...
        InventarioFecha: Decimal;
        Item2: Record Item;
        Window: Dialog;
        CounterTotal: Integer;
        Counter: Integer;
        Text001: Label 'Procesando Producto    #1########## @2@@@@@@@@@@@@@';
        Text002: Label 'Se han procesado %1 productos de un total de %2.';
        CounterOK: Integer;
        EAN: Text[100];
        itemcross_g: Record "Item Reference";
}
