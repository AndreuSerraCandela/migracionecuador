table 76136 "Colegio - Adopciones Cab"
{
    ...
    fields
    {
        ...
        field(4; "Cod. Promotor"; Code[20])
        {
            ...
            trigger OnValidate()
            var
                TeamSP: Record "Team Salesperson";
                Team: Record Team;
            begin
                TeamSP.Reset;
                TeamSP.SetRange("Salesperson Code", "Cod. Promotor");
                if TeamSP.FindFirst then begin
                    Team.Get(TeamSP."Team Code");
                    Campana := Team.Campaña;
                end;
            end;
        }
        ...
        field(25; "% Dto. Donacion Plantel"; Decimal)
        {
            CaptionClass = '5,6,1';

            trigger OnValidate()
            begin
                if "% Dto. Donacion Plantel" <> xRec."% Dto. Donacion Plantel" then begin
                    if Confirm(Msg001, false) then begin
                        AdopcionesD.Reset;
                        AdopcionesD.SetRange("Cod. Colegio", "Cod. Colegio");
                        AdopcionesD.SetRange("Cod. Local", "Cod. Local");
                        ...
                        if AdopcionesD.FindSet(true) then
                            repeat
                                AdopcionesD."% Dto. Donacion Plantel" := "% Dto. Donacion Plantel";
                                AdopcionesD.Modify;
                            until AdopcionesD.Next = 0;
                    end;
                end;
